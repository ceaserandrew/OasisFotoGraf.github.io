...
export default function AIPanel({ album, photosLength, photoNames }: AIPanelProps) {
  ...
  return (
    <div className="w-full bg-[#111] border border-white/10 rounded-none overflow-hidden shadow-2xl h-auto lg:h-full flex flex-col font-sans max-h-[600px] lg:max-h-none">
      {/* Header bar panel */}
      <div className="bg-[#0A0A0A] border-b border-white/10 px-4 sm:px-5 py-3 sm:py-4 flex items-center justify-between gap-3">
        ...
        <div className="flex bg-white/5 border border-white/10 rounded-none p-0.5 text-[9px] font-mono tracking-wider">
          <button
            ...
            className={`px-3 sm:px-4 py-2 sm:py-1 rounded-none transition-colors cursor-pointer uppercase min-h-[40px] min-w-[80px] ${
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-5 custom-scrollbar bg-black/5">
          ...
          {currentTab === "story" ? (
            ...
            >
              {loadingStory ? (
                ...
                <div className="text-center py-12 sm:py-16 text-zinc-550">
                ...
                <div className="prose prose-invert h-full">
                  ...
                  <div className="text-xs sm:text-sm text-zinc-300 font-serif italic leading-relaxed border-l border-zinc-650 pl-4 py-1.5 mb-5 space-y-2">
                    <Quote className="w-3.5 h-3.5 text-zinc-650 mb-1.5" />
                    <span className="whitespace-pre-line block text-[#CECECE] font-light tracking-wide leading-relaxed">{story}</span>
                  </div>
      ...
      {currentTab === "chat" && (
        <div className="p-3 sm:p-4 bg-[#0A0A0A] border-t border-white/10 flex flex-col gap-3">
          {/* Action buttons (Clear chat) */}
          <div className="flex items-center justify-between">
            <button
              onClick={handleClearChat}
              className="flex items-center gap-1 text-[9px] font-mono text-zinc-500 hover:text-white transition-colors cursor-pointer tracking-wider uppercase min-h-[40px] px-2"
          ...
          <form onSubmit={handleSendMessage} className="flex gap-2">
            <input
              ...
              className="flex-1 bg-[#121212] border border-white/5 rounded-none px-3 py-2.5 text-xs text-zinc-200 focus:outline-hidden focus:border-white/20 select-text min-h-[44px]"
            />
            <button
              type="submit"
              disabled={loadingChat || !inputMessage.trim()}
              className="p-2.5 sm:p-3 bg-white text-black hover:bg-[#D1D1D1] rounded-none transition-colors cursor-pointer disabled:opacity-50 min-h-[44px] min-w-[44px]"
            >
