  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#090909]/95 [backdrop-filter:blur(8px)] select-none font-sans">
        ...
        <div className="absolute top-0 left-0 w-full p-5 flex items-center justify-between bg-gradient-to-b from-black/80 to-transparent z-40 pointer-events-none">
          <div className="text-zinc-400 text-[10px] font-mono tracking-[0.15em] uppercase pointer-events-auto flex items-center gap-2 bg-[#121212]/90 px-4.5 py-2 border border-white/15">
            <span>SPECIMEN :</span>
          ...
          <div className="flex items-center gap-2 pointer-events-auto">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="p-2.5 rounded-none bg-white/5 border border-white/10 text-zinc-400 hover:text-white hover:border-white/20 transition-colors cursor-pointer"
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="p-2.5 rounded-none bg-white/5 border border-white/10 text-zinc-400 hover:text-white transition-colors cursor-pointer"
            ...
            <button
              onClick={onClose}
              className="p-2.5 rounded-none bg-white/5 border border-white/10 text-zinc-400 hover:text-white hover:border-white/30 transition-colors cursor-pointer ml-1"
        ...
        {/* Carousel Prev Navigation Pin Left */}
        <button
          onClick={(e) => {
            e.stopPropagation();
            onPrev();
          }}
          className="absolute left-6 top-1/2 -translate-y-1/2 p-3.5 rounded-none bg-[#121212]/80 border border-white/10 text-zinc-400 hover:text-white hover:border-white/20 z-30 transition-all cursor-pointer hidden sm:flex"
        >
          <ChevronLeft className="w-5 h-5" />
        </button>

        {/* Main Immersive Canvas Area */}
        <div className="relative w-full h-full max-w-5xl max-h-[80%] flex items-center justify-center p-5 z-20">
          <motion.img
        ...
        {/* Carousel Next Navigation Pin Right */}
        <button
          onClick={(e) => {
            e.stopPropagation();
            onNext();
          }}
          className="absolute right-6 top-1/2 -translate-y-1/2 p-3.5 rounded-none bg-[#121212]/80 border border-white/10 text-zinc-400 hover:text-white hover:border-white/20 z-30 transition-all cursor-pointer hidden sm:flex"
        >
        ...
        <AnimatePresence>
          {showMeta && (
            <motion.div
              ...
              exit={{ opacity: 0, y: 25 }}
              className="absolute bottom-6 left-1/2 -translate-x-1/2 w-full max-w-xl px-5 z-30 pointer-events-none"
            >
              <div className="relative bg-[#111]/95 border border-white/10 rounded-none p-5 shadow-2xl backdrop-blur-md pointer-events-auto">
                <div className="flex flex-col gap-3">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-[9px] font-mono tracking-wider uppercase text-zinc-450">
        ...
        <div className="absolute bottom-6 left-5 text-[9px] font-mono uppercase tracking-widest text-zinc-600 hidden max-sm:block select-none">
